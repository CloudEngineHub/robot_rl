...
namespace obelisk::viz {
    template <typename EstimatorMessageT> class RobotViz : public ObeliskVizRobot<EstimatorMessageT> {
      public:
        explicit RobotViz(const std::string& name) : ObeliskVizRobot<EstimatorMessageT>(name) {}
      ...
      protected:
        void ParseEstimatedState(const EstimatorMessageT& msg) override {
            ...
            this->base_tf_.header.frame_id =
                "odom_frame"; // TODO: Consider making this not hardcoded. This must match the rviz fixed frame
