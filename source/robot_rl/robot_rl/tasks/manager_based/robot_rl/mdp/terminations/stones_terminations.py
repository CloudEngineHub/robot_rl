...
def finished_long_stones(env, stones_command_name: str, output_command_name: str) -> torch.Tensor:
    """
    ...
    stones_command = env.command_manager.get_term(stones_command_name)
    ...
    end_stone_pos_xz = torch.stack([stones_command.abs_x[:, -1], 
                                 stones_command.abs_z[:, -1]], dim=1) # (num_envs, 2)
...
def long_stones_deviation(env, stones_command_name: str, output_command_name: str) -> torch.Tensor:
    """
    ...
    end_stone_pos_y = stones_command.abs_y[:, -1]  # (num_envs,)
